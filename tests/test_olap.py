...
class TestLoadDim:
    ...
    async def test_load_product_dim_success(self, session: AsyncSession):
        ...
        result = await session.execute(text("SELECT * FROM dwh.Product_Dim"))
        ...
        count_res = await session.execute(text("SELECT COUNT(*) FROM dwh.Product_Dim"))
        assert count_res.scalar() == 2

        result = await session.execute(
            text("SELECT * FROM dwh.Product_Dim WHERE is_current = False")
        )
        ...
        result = await session.execute(
            text("SELECT * FROM dwh.Product_Dim WHERE is_current = True")
        )
    ...
    async def test_load_customer_dim_success(self, session: AsyncSession):
        ...
        result = await session.execute(text("SELECT * FROM dwh.Customer_Dim"))
        ...
        count_res = await session.execute(text("SELECT COUNT(*) FROM dwh.Customer_Dim"))
        assert count_res.scalar() == 2

        result = await session.execute(
            text("SELECT * FROM dwh.Customer_Dim WHERE is_current = False")
        )
        ...
        result = await session.execute(
            text("SELECT * FROM dwh.Customer_Dim WHERE is_current = True")
        )
...
class TestLoadFact:
    ...
    async def test_load_sales_fact_success(self, session: AsyncSession):
        ...
        result = await session.execute(text("SELECT * FROM dwh.Sales_Fact"))
        sales_fact = result.fetchone()
        assert sales_fact

        await load_sales_fact(session)
        result = await session.execute(text("SELECT COUNT(*) FROM dwh.Sales_Fact"))
    ...
    async def test_load_sales_fact_correct_dim_selection(self, session: AsyncSession):
        ...
        first_sale_result = await session.execute(
            text("SELECT * FROM dwh.Sales_Fact WHERE sale_id = :sale_id"),
