...
class TestOLTP:
    ...
    async def test_insert_product_success(self, session):
        ...
        res = await session.execute(text("SELECT count(*) FROM source.Products"))
        assert res.scalar() == 1
    ...
    async def test_update_product_success(self, session):
        ...
        res = await session.execute(text("SELECT category FROM source.Products"))
    ...
    async def test_insert_customer_success(self, session):
        ...
        res = await session.execute(text("SELECT count(*) FROM source.Customers"))
        assert res.scalar() == 1
    ...
    async def test_update_customer_success(self, session):
        ...
        res = await session.execute(text("SELECT phone FROM source.Customers"))
    ...
    async def test_insert_sale_success(self, session):
        ...
        res = await session.execute(text("SELECT count(*) FROM source.Sales"))
        assert res.scalar() == 1
